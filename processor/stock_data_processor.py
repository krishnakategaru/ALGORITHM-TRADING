...
from sentiment_analysis.sentiment_analysis_2_model import train_model
class StockDataProcessor:
    def __init__(self, stock_ticker, start_date, end_date, news_sentiment,ohlc_sentiment,ohlc_ta_sentiment):
        ...
        self.model_1_sentiment = ohlc_sentiment
        self.model_2_sentiment = ohlc_ta_sentiment
    ...
    def preprocess_sentiment_data(self):
        ...
        if self.model_1_sentiment:
            transformer_sentiment_data,_ = prepare_sentiment_from_transformer(self.stock_ticker, self.start_date, self.end_date,'xgboost')
            print(transformer_sentiment_data.columns)
            transformer_sentiment_data = transformer_sentiment_data["model_1_sentiment"].copy()
        ...
        if self.model_2_sentiment :
            model_2_data,_ = train_model(self.stock_ticker, self.start_date, self.end_date,batch_size= 256, sequence_length= 30,stride = 1)
            # print(transformer_sentiment_data.columns)
            model_2_data = model_2_data["model_2_sentiment"].copy()
            # transformer_sentiment_data['date'] = transformer_sentiment_data.iloc[:,0]
            if self.news_sentiment or self.model_1_sentiment:
                merged_df = pd.merge(merged_df, model_2_data,left_index=True, right_index = True, how ='left')
            else :
                merged_df = pd.merge(self.data, model_2_data, left_index=True, right_index = True, how='left')
                # merged_df.set_index('date', inplace=True)
